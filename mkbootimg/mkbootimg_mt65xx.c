...
int mt65xx_write_padding(int fd, unsigned pagesize, unsigned itemsize)
{
...
int mkbootimg_mt65xx_main(int argc, char **argv)
{
    ...
    if(mt65xx_write_padding(fd, pagesize, sizeof(hdr))) goto fail;

    if(write(fd, &kernel_hdr, 512) != 512) goto fail;
    if(write(fd, kernel_data, kernel_hdr.kernel_size) != kernel_hdr.kernel_size) goto fail;
    if(mt65xx_write_padding(fd, pagesize, hdr.kernel_size)) goto fail;

    if(write(fd, &rootfs_hdr, 512) != 512) goto fail;
    if(write(fd, ramdisk_data, rootfs_hdr.rootfs_size) != rootfs_hdr.rootfs_size) goto fail;
    if(mt65xx_write_padding(fd, pagesize, hdr.ramdisk_size)) goto fail;
